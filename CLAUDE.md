...
- Озвучка **по умолчанию только по кнопке**. Автопроизношение при показе ответа есть, но это
  осознанный выбор человека: галка `Voorlezen` в строке вердикта, по умолчанию снята и
  запоминается в localStorage (`core/settings.ts`). Само приложение в тихом месте не заговорит —
  ровно поэтому включённой по умолчанию она быть не может. Настройка общая для всех карточек
  и режимов: это привычка, а не свойство вопроса.
- Галка озвучивает **только слово, не пример**. Пример читается отдельной кнопкой после него:
  слово плюс фраза на каждой карточке — это несколько секунд звука, и темп сессии просядет.
  Фраза нужна, когда её хочется разобрать, а не всегда. Синтезатору отдаётся только
  нидерландская половина примера: русский перевод нидерландский голос прочёл бы по буквам.
...
- Dev-сервер живёт на порту 4400: порт прописан в `serve.options` в `angular.json`, поэтому
  `npm start`, `ng serve` и запуск из IDE согласованы. 4200 занят проектом Design Canvas, 4300
  освобождён (там в IndexedDB осталась база несовместимой версии).
- Там же `"host": "127.0.0.1"`, и убирать его нельзя. По умолчанию `ng serve` слушает только
  `[::1]`, а на этой машине IPv6-петля не завершает рукопожатие: сокет виден в `netstat` как
  LISTENING, но соединения зависают в `SYN_SENT`, и браузер показывает `ERR_CONNECTION_TIMED_OUT`
  вместо честного отказа. `localhost` резолвится в `::1`, поэтому **открывать по
  `http://127.0.0.1:4400/`**, а не по `localhost`. Проверено на двух независимых серверах: с
  соседним проектом на 4200 то же самое, так что дело в машине, а не в проекте.
- PWA-сборка раздаётся с 4500, а не с 4400: service worker привязан к origin и, оставшись
  зарегистрированным, перехватывал бы запросы dev-сервера и отдавал старые ассеты. Разные
  порты — это ещё и разные IndexedDB, так что тестовые данные PWA не мешают разработке.
...
- **Резюме чата по просьбе сохраняется файлом, а не только печатается.** Память привязана
  к пути проекта, и сессия, начатая в другом каталоге, переписки не увидит — а файл в
  репозитории поедет с ним. Кладётся в тот репозиторий, о работе в котором речь, как документ
  состояния (`NEXT.md`), а не протокол разговора: состояние дел, замеры, которые не надо
  повторять, следующий шаг, особенности машины, открытые хвосты. То, что уже описано в
  `CLAUDE.md`, `NOTICE.md` или `SCHEMA.md`, не дублируется — вместо этого ссылка. Файл
  остаётся в индексе, коммит за человеком, а в терминале печатается сжатая версия, чтобы
  не открывать файл ради чтения.
...
- `src/app/core/quiz.ts` — сборка вопросов. Узнавание и припоминание разведены по режимам:
  `Quiz.forToday` спрашивает только `nl-ru` (узнать), `Quiz.forRecall` — только `ru-nl`
  (вспомнить, роут `/recall`). Случайное чередование осталось лишь в тренировке параграфа
  (`forChapter`): там задача познакомиться с параграфом, а не гонять один навык.
...
- **Прогресс разделён по навыкам** (`Progress.skill`: `translation` | `recall` | `article` | `verb`), ключ составной
...
- Узнавание (`translation`, `nl-ru`) и припоминание (`recall`, `ru-nl`) — **разные навыки
  с разными коробками**, по той же причине, по которой отделён артикль: припоминание труднее,
  и слово, уверенно узнаваемое в тексте, в речи вспоминается далеко не всегда. С общей записью
  угаданное узнавание отодвигало бы срок припоминания, которое ни разу не спрашивали.
  Цена решения — вдвое больше материала к повторению, и это осознанно.
- Отбор «сначала по срокам, потом новые» живёт в одном месте (`Quiz.dueThenFresh`). Раньше он
  был скопирован в каждый режим, и одна из копий разошлась с остальными: читала прогресс без
  фильтра по навыку. Новый режим обязан брать слова через него же.
...
```
npm run pwa:serve     сборка + статический сервер на :4500 (service worker живёт только здесь,
                      в ng serve его нет)
